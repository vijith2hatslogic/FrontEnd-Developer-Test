...
import { storageService, Task } from '@/lib/storage'
...
export default function CreateTest() {
  ...
  const { register, handleSubmit, control, formState: { errors }, watch, setValue, getValues } = useForm<TestFormData>({
  ...
  const watchTechStacks = watch('techStacks')
  const watchExperienceLevel = watch('experienceLevel')
  const watchTotalTime = watch('totalTime')
  
  // Auto-generate tasks when tech stacks, experience level, or total time changes
  useEffect(() => {
    if (watchTechStacks && watchTechStacks.length > 0) {
  ...
  }, [watchTechStacks, watchExperienceLevel, watchTotalTime, replace])
  
  // Generate a title based on selected tech stacks
  useEffect(() => {
    if (watchTechStacks && watchTechStacks.length > 0) {
      ...
      if (!currentTitle) {
        ...
        if (watchTechStacks.length <= 3) {
          // If 3 or fewer tech stacks, include them all in the title
          newTitle = `${watchTechStacks.join('/')} Developer Assessment`
        } else {
          // If more than 3, use a more generic title
          newTitle = 'Front-end Developer Assessment'
          
          // Check for specific combinations
          if (watchTechStacks.includes('React') || watchTechStacks.includes('Vue') || 
              watchTechStacks.includes('Angular') || watchTechStacks.includes('Svelte')) {
            newTitle = 'JavaScript Framework Developer Assessment'
          }
          
          if (watchTechStacks.includes('Next.js') || watchTechStacks.includes('Nuxt.js')) {
            newTitle = 'Full-stack JavaScript Developer Assessment'
          }
        }
        
        // Add experience level to title
        if (watchExperienceLevel.includes('Junior')) {
          newTitle = `Junior ${newTitle}`
        } else if (watchExperienceLevel.includes('Senior')) {
          newTitle = `Senior ${newTitle}`
        }
        
        setValue('title', newTitle)
      }
    }
  }, [watchTechStacks, watchExperienceLevel, setValue, getValues])
  ...
  const onSubmit = async (data: TestFormData) => {
    ...
    try {
      ...
      // Create test using storage service
      const newTest = await storageService.createTest({
        ...data,
        createdBy: user.id,
      })
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-sm text-info mt-2">
                  Tasks will be automatically generated based on selected tech stacks.
                </p>
                {errors.techStacks && (
